...
import org.apache.spark.sql.sources.{BaseRelation, PrunedFilteredScan, TableScan}
...
private[hbase] case class HbaseRelation(@transient val hbaseProps:Map[String , String])
                   (@transient val sqlContext:SQLContext) extends BaseRelation with TableScan /*with PrunedFilteredScan*/{
  ...
  /*val hbaseConf = HBaseConfiguration.create()
    hbaseConf.set("hbase.zookeeper.quorum", constans.hbase_zookeeper_quorum)//指定zookeeper的地址
